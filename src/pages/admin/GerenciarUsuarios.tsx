...
import { usuarios, criarUsuario, atualizarUsuario, excluirUsuario } from '@/data/seed'
...
export function GerenciarUsuarios() {
  ...
  const usuariosFiltrados = usuarios
    ...
    .filter((u) => {
      ...
      return (u.nome || '').toLowerCase().includes(termo) || (u.email || '').toLowerCase().includes(termo) || u.role.includes(termo)
    })

  function handleSave() {
    if (!form.nome.trim() || !form.email.trim()) { toast.error('Preencha nome e email'); return }
    if (editingId) {
      atualizarUsuario(editingId, { nome: form.nome, email: form.email, role: form.role, telefone: form.telefone })
      toast.success('Usuario atualizado!')
    } else {
      criarUsuario({ nome: form.nome, email: form.email, role: form.role, telefone: form.telefone })
  ...
  function handleEdit(u: (typeof usuarios)[0]) {
    setForm({ nome: u.nome, email: u.email, role: u.role, telefone: u.telefone || '' })
    setEditingId(u.id)
    setShowForm(true)
  }
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-secondary">{usuarios.filter((u) => u.ativo).length} usuarios cadastrados</p>
        </div>
        <Button onClick={() => { setForm({ nome: '', email: '', role: 'aluno', telefone: '' }); setEditingId(null); setShowForm(!showForm) }}>
          <Plus size={18} className="mr-2" /> Novo Usuario
        </Button>
      </div>
      ...
      {showForm && (
        <GlassCard className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as any })} className="glass-input w-full text-sm">
