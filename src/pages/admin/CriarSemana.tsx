...
export function CriarSemana() {
  ...
  function handleEdit(s: Semana) {
    setForm({ id: s.id, fase_id: s.fase_id, nome: s.nome, tipo: s.tipo as any, ordem: s.ordem, descricao: s.descricao || '' })
    setShowForm(true)
  ...
  const semanasAtivas = semanas.filter(s => s.ativa)

  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Semanas de Treino</h1>
          <p className="text-sm text-text-secondary">{semanasAtivas.length} semanas criadas</p>
        </div>
        <Button onClick={() => { setShowForm(!showForm); setForm({ fase_id: '', nome: '', tipo: 'ORDINARIA', ordem: 1, descricao: '' }) }}>
      ...
      {showForm && (
        <GlassCard className="p-5 space-y-4">
          <h3 className="font-semibold text-text-primary">Nova Semana</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <select value={form.fase_id} onChange={e => setForm(prev => ({ ...prev, fase_id: e.target.value }))} className="glass-input w-full text-sm">
              <option value="">Selecione a fase...</option>
              {fases.filter(f => f.ativa).map(f => (
                <option key={f.id} value={f.id}>{f.nome} (Ordem {f.ordem})</option>
              ))}
            </select>
            <Input value={form.nome} onChange={e => setForm(prev => ({ ...prev, nome: e.target.value }))} placeholder="Nome da semana" />
            <select value={form.tipo} onChange={e => setForm(prev => ({ ...prev, tipo: e.target.value }))} className="glass-input w-full text-sm">
              {tiposSemana.map(t => <option key={t} value={t}>{t.replace('_', ' ')}</option>)}
            </select>
            <Input type="number" value={form.ordem} onChange={e => setForm(prev => ({ ...prev, ordem: parseInt(e.target.value) || 1 }))} placeholder="Ordem" />
      ...
      <div className="space-y-6">
        {fases.filter(f => f.ativa).map((f) => {
          const semanasFase = semanas.filter(s => s.fase_id === f.id && s.ativa)
          ...
          return (
            <div key={f.id} className="space-y-3">
              ...
              {semanasFase.length === 0 ? (
                <p className="text-xs text-text-secondary ml-6">Nenhuma semana criada nesta fase.</p>
              ) : (
                  ...
                  {semanasFase.map(s => (
                    <GlassCard key={s.id} className="p-4 space-y-2">
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="text-sm font-semibold text-text-primary">{s.nome}</p>
                          <p className="text-xs text-text-secondary">{s.tipo} — Ordem {s.ordem}</p>
                        </div>
                        <div className="flex gap-1">
                          <button onClick={() => handleEdit(s)} className="p-1.5 rounded-lg hover:bg-white/[0.03]">
                            <Edit2 size={14} className="text-text-secondary" />
                          </button>
                          <button onClick={() => handleDelete(s.id)} className="p-1.5 rounded-lg hover:bg-error/5">
                            <Trash2 size={14} className="text-error" />
                          </button>
                        </div>
                      </div>
                      <div className="flex gap-2 mt-2">
                        <Badge variant={s.tipo === 'DELOAD' ? 'warning' : s.tipo === 'PEAK' ? 'accent' : 'default'}>
                          {s.tipo.replace('_', ' ')}
                        </Badge>
                        <Badge variant="default">{diasMap[s.id] || 0} dias</Badge>
                      </div>
                    </GlassCard>
                  ))}
                </div>
