...
async function getAlunoIdFromAuth(): Promise<string | null> {
...
export function CampeonatosAluno() {
  ...
  useEffect(() => {
    async function carregar() {
      try {
        ...
        const id = await getAlunoIdFromAuth()
        setAlunoId(id)
  ...
  async function confirmarInscricao() {
    if (!alunoId) {
      toast.error('Aluno nao encontrado. Verifique seu cadastro.')
      return
    }
    ...
    if (error) {
      console.error('Erro inscricao:', error)
      toast.error('Erro ao salvar inscricao.')
      return
    }

    toast.success('Inscricao salva com sucesso!')
  ...
  function jaInscrito(campeonatoId: string) {
    return minhasInscricoes.some((i) => i.campeonato_id === campeonatoId)
  }

  async function abrirModal(camp: any) {
    if (!alunoId) {
      toast.error('Aluno nao encontrado. Verifique seu cadastro.')
      return
    }
    ...
    if (data) {
      ...
      setParceiro1(data.parceiro1 || '')
      setParceiro2(data.parceiro2 || '')
      setParceiro3(data.parceiro3 || '')
      setParceiro4(data.parceiro4 || '')
      setParceiro5(data.parceiro5 || '')
          ...
          <GlassCard key={camp.id} className="p-5">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <h2 className="text-xl font-bold text-text-primary">{camp.nome}</h2>
                <p className="text-text-secondary">{camp.descricao}</p>
            ...
            >
              {jaInscrito(camp.id) ? 'Editar Inscricao' : 'Vou Participar'}
            </button>
          </GlassCard>
        ))
      )}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <GlassCard className="w-full max-w-lg p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold text-text-primary">
              {inscricaoAtual ? 'Editar Inscricao' : 'Nova Inscricao'}
            </h2>
            <p className="text-text-secondary">{campeonatoSelecionado?.nome}</p>
            ...
            <div className="space-y-4">
              ...
              {modalidade !== 'Individual' && modalidade !== '' && (
                <input
                  value={parceiro1}
              ...
              {(modalidade === 'Trio' || modalidade === 'Quarteto' || modalidade === 'Sexteto') && (
                <input
                  value={parceiro2}
              ...
              {(modalidade === 'Quarteto' || modalidade === 'Sexteto') && (
                <input
                  value={parceiro3}
              ...
              <div className="flex justify-end gap-3 mt-4">
                ...
                >
                  {inscricaoAtual ? 'Salvar Alteracoes' : 'Confirmar Inscricao'}
                </button>
              </div>
            </div>
          </GlassCard>
