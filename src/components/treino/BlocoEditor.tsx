...
export function BlocoEditor({ value, onChange }: BlocoEditorProps) {
  ...
  function moverBloco(id: string, direction: number) {
    setBlocos(prev => {
      ...
      if (
        ...
        (direction > 0 && currentIndex === prev.length - 1)
      ) {
        return prev
      }
      ...
      const currentBlock = { ...next[currentIndex] }
      const targetBlock = { ...next[targetIndex] }

      next[currentIndex] = targetBlock
    ...
    setBlocos(prev =>
      prev.map(b => {
        ...
        return {
          ...
          exercicios: [
            ...b.exercicios,
            novoExercicio(b.exercicios.length)
          ]
        }
      })
    )
  ...
  function updEx(blocoId: string, exId: string, patch: Partial<ExercicioBloco>) {
    setBlocos(prev =>
      prev.map(b => {
        if (b.id !== blocoId) return b
        return {
